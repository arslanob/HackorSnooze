...
class Story {
  ...
  getHostName() {
    ...
    const storyHostname = storyURL.hostname;
    return storyHostname;
  }
...
class StoryList {
  ...
  async removeStory(user, storyId){
    ...
    //find the storyid we want to remove.
    this.stories = this.stories.find(s => s.storyId === storyId);
    //filter out the story we want to remove from ownstories and favorites.
...
class User {
  ...
  async updateUser(usernewusername) {
    const token = this.loginToken;
    const response = await axios({
      url: `${BASE_URL}/users/${this.username}`,
      method: "PATCH",
      data: { token, user: { username: `${usernewusername}` } },
    });
